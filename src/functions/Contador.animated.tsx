...
export function DigitoAnimado({ valor, duracao = 200 }: DigitoAnimadoProps) {
  ...
  return (
    <View style={{ position: 'relative', height: 100, width: 50, overflow: 'hidden' }}>
      <Animated.Text style={[styles.texto, {position: 'absolute', transform: [{ translateY: translateAnterior }], opacity: opacityAnterior}]}>
        {digitoAnterior.current}
      </Animated.Text>
      <Animated.Text style={[styles.texto, {position: 'absolute', transform: [{ translateY: translateNovo }], opacity: opacityNovo}]}>
        {digitoAtual}
      </Animated.Text>
    </View>
  );
